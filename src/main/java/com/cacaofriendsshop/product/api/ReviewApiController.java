...
public class ReviewApiController {
    ...
    @PostMapping
    public ResponseEntity<Review> create(@RequestBody ReviewRequestDto reviewRequestDto) {
        Review saved = reviewService.save(reviewRequestDto);
        return ResponseEntity.created(URI.create("/" + saved.getId())).body(saved);
    }

    @PutMapping
    public ResponseEntity<Review> update(@RequestBody ReviewRequestDto reviewRequestDto) {
